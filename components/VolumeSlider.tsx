...
const VolumeSlider = React.forwardRef<
  ...
  VolumeSliderProps
>(({ className, volumeLevel = 0, ...props }, ref) => (
  ...
  >
    <SliderPrimitive.Track className="relative h-6 w-full grow overflow-hidden rounded-full bg-secondary">
      {/* Volume Level Meter */}
      <div
        className="absolute h-full bg-gradient-to-r from-green-500 via-yellow-500 to-red-500 opacity-20 transition-all duration-100"
        style={{ width: `${volumeLevel}%` }}
      />
      {/* Volume Control Track */}
      <SliderPrimitive.Range className="absolute h-full bg-primary opacity-50" />
